<script lang="ts">
    ...
    const channelsByCategory = $state<
        { category: string; channels: Channel[] }[]
    >([
        {
    ...
    onWsMessage(async (type, data) => {
        if (serverToClientMsgHandlers[type])
            await serverToClientMsgHandlers[type](data);
    });
</script>

<svelte:window
    onfocus={() => (isUserInPage = true)}
    onblur={() => (isUserInPage = false)}
/>
...
>
    <aside class="flex w-16 flex-col gap-2 border-r p-2">
        <img src={pexe} alt="" class="h-12 w-12" />
        <img src={pexe} alt="" class="h-12 w-12" />
        <img src={pexe} alt="" class="h-12 w-12" />
        <img src={pexe} alt="" class="h-12 w-12" />
        <img src={pexe} alt="" class="h-12 w-12" />
    ...
    <aside class="flex w-60 flex-col border-r p-2">
            ...
            {#each channelsByCategory as { category, channels }}
                ...
                {#each channels as channel}
                    <div>
                        ...
                        <div
                            class="pl-8 {channel.type == 'voice' &&
                            voiceChannels[channel.id]?.length > 0
                                ? 'mb-2'
                                : ''}"
                        >
                            {#if channel.type == "voice"}
                                {#each voiceChannels[channel.id] as user}
                                    <button
                                        class="flex w-full items-center gap-2 p-0.5 text-left hover:bg-gray-800"
                                    >
                                        <img
                                            src={pexe}
                                            alt=""
        ...
        <div class="flex flex-col gap-2">
            ...
            {/if}
            <button onclick={() => setMuted(!currentChannel.isMuted)}>
        ...
        <ul bind:this={messagesEl} class="flex h-full flex-col overflow-auto">
            {#each messages as message, i}
                ...
                <li
                    class="{sameAuthor
                        ? ''
                        : 'mt-2'} flex items-center gap-2 break-all hover:bg-gray-800"
                    oncontextmenu={(e) => {
                ...
                >
                    {#if !sameAuthor}
                        <img src={pexe} alt="" class="h-10 w-10" />
                    {/if}
                    ...
                    <span class="flex flex-col items-start justify-start">
                        ...
                        <span
                            class="whitespace-pre-wrap break-all {sameAuthor
                                ? 'ml-12'
    ...
    <aside class="flex w-60 flex-col gap-2 border-l p-2">
        {#each orderedUserStatuses as user}
            <div class="flex items-center gap-2">
                <img src={pexe} alt="" class="h-8 w-8" />

                <div
                    class="rounded-full {user.afk
                        ? 'bg-yellow-500'
                        : 'bg-green-500'}  p-1"
                ></div>

                <span
                    class="line-clamp-1 break-all {user.afk
                        ? 'text-gray-400'
